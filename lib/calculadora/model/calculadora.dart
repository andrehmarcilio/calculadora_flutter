class Calculadora {
  ...
  String formatarNumeroInteiro(double tavlezInteiro) {
    return tavlezInteiro - tavlezInteiro.round() == 0
        ? tavlezInteiro
            .toString()
            .substring(0, tavlezInteiro.toString().length - 2)
  ...
  limparUm() {
    ...
    if (stringNumeroAtual.isEmpty) {
      if (operadores.length ==
          (colchetesIndex.isNotEmpty ? colchetesIndex.last : 0)) {
      ...
      if (operadores.length ==
              (colchetesIndex.isNotEmpty ? colchetesIndex.last : -1) &&
          colchetesIndex.length % 2 == 0) {
        colchetesIndex.removeLast();

        return;
      }
      stringNumeroAtual =
          stringNumeroAtual.substring(0, stringNumeroAtual.length - 1);
    }
  }
  ...
  calcular() {
    ...
    for (int i = 0; i < colchetesIndex.length; i += 2) {
      ...
      numeros[colchetesIndex[i + 1]] = double.parse(
          realizarOperacoes(operadoresColchetes, numerosColchetes));
    }
